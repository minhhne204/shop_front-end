  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-10 animate-fade-in">
      <nav className="flex items-center gap-2 text-[14px] mb-8">
        ...
        <span className="text-[#2D2D2D] font-medium truncate max-w-[200px]">
          {product.name}
        </span>
      </nav>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
        ...
        <div>
          <div className="flex items-center gap-3 mb-4">
            {statusLabel && (
              <span
                className={`${statusLabel.bg} text-white text-[12px] font-medium px-3 py-1 rounded-full`}
              >
                {statusLabel.text}
              </span>
            )}
          ...
          <h1 className="text-[26px] font-semibold text-[#2D2D2D] leading-tight mb-5">
            {product.name}
          </h1>
          ...
          {product.hasVariants && product.variants?.length > 0 && (
            <div className="mb-6">
              <span className="text-[14px] text-[#6B6B6B] block mb-3">
                {product.variantType || "Phiên bản"}:{" "}
                {selectedVariant ? (
                  <span className="text-[#2D2D2D] font-medium">
                    {selectedVariant.name}
                  </span>
                ) : (
                  <span className="text-[#C45C4A]">Chọn phiên bản</span>
                )}
              </span>
            ...
            <div className="flex items-center border border-[#EBEBEB] rounded-xl overflow-hidden">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setQuantity(Math.min(Math.max(1, value), getCurrentStock()));
                }}
              ...
              <button
                ...
                className="w-11 h-11 flex items-center justify-center text-[#6B6B6B]
    ...
    disabled:opacity-40 disabled:cursor-not-allowed"
          ...
          {message.text && (
            <div
              className={`text-center py-3 px-4 rounded-xl text-[14px] ${
                message.type === "error"
          ...
          <div className="border-t border-[#EBEBEB] pt-6 mt-6 space-y-3">
            <div className="flex items-center gap-2 text-[14px]">
              ...
              <Link
                to={`/san-pham?category=${product.category?._id}`}
                className="text-[#7C9A82] hover:text-[#6B8A71] font-medium transition-colors"
              >
            ...
            <div className="flex items-center gap-2 text-[14px]">
              ...
              <Link
                to={`/san-pham?brand=${product.brand?._id}`}
                className="text-[#7C9A82] hover:text-[#6B8A71] font-medium transition-colors"
              >
                {product.brand?.name}
              </Link>
          ...
          <div className="grid grid-cols-3 gap-4 mt-8">
            <div className="text-center p-4 bg-[#F5F5F3] rounded-xl">
              <svg
                className="w-6 h-6 mx-auto mb-2 text-[#7C9A82]"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
      ...
      {product.description && (
        <div className="bg-white rounded-2xl border border-[#EBEBEB] p-8 mb-8">
          <h2 className="text-[20px] font-semibold text-[#2D2D2D] mb-5">
            Mô tả sản phẩm
          </h2>
      ...
      {relatedProducts.length > 0 && (
        <div>
          <h2 className="text-[22px] font-semibold text-[#2D2D2D] mb-8">
            Sản phẩm liên quan
          </h2>
