  ...
  if (!user) {
    return (
      <div className="max-w-6xl mx-auto px-6 py-20 text-center animate-fade-in">
        ...
        <h1 className="text-[24px] font-semibold text-[#2D2D2D] mb-3">Giỏ hàng</h1>
        <p className="text-[15px] text-[#6B6B6B] mb-8">Vui lòng đăng nhập để xem giỏ hàng</p>
        <Link
          to="/dang-nhap"
  ...
  if (cart.items.length === 0) {
    return (
      <div className="max-w-6xl mx-auto px-6 py-20 text-center animate-fade-in">
        ...
        <h1 className="text-[24px] font-semibold text-[#2D2D2D] mb-3">Giỏ hàng trống</h1>
        <p className="text-[15px] text-[#6B6B6B] mb-8">Bạn chưa có sản phẩm nào trong giỏ hàng</p>
        <Link
          to="/san-pham"
          className="inline-block bg-[#2D2D2D] text-white px-8 py-3.5 rounded-xl text-[15px] font-medium hover:bg-[#7C9A82] transition-colors"
        >
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-10 animate-fade-in">
      <h1 className="text-[28px] font-semibold text-[#2D2D2D] mb-8">
        Giỏ hàng <span className="text-[#6B6B6B] font-normal">({itemCount} sản phẩm)</span>
      </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-2xl border border-[#EBEBEB] p-6 sticky top-[100px]">
            <h2 className="text-[16px] font-semibold text-[#2D2D2D] mb-5">Tổng đơn hàng</h2>
            <div className="space-y-4 text-[14px]">
              <div className="flex justify-between">
                <span className="text-[#6B6B6B]">Tạm tính</span>
                <span className="text-[#2D2D2D] font-medium">{formatPrice(cartTotal)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[#6B6B6B]">Phí vận chuyển</span>
                <span className={shippingFee === 0 ? 'text-[#7C9A82] font-medium' : 'text-[#2D2D2D]'}>
              ...
              <div className="border-t border-[#EBEBEB] pt-4 flex justify-between">
                ...
                <span className="text-[18px] font-semibold text-[#C45C4A]">{formatPrice(cartTotal + shippingFee)}</span>
              </div>
            </div>
            <button
