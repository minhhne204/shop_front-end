...
const AdminOrders = () => {
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      await api.put(`/admin/orders/${updateModal._id}/status`, {
        status: newStatus,
      });
      setUpdateModal(null);
      setNewStatus("");
                  ...
                  {orders.map((order) => (
                    ...
                    >
                      <td className="px-5 py-4">
                        <p className="text-[14px] font-medium text-[#7C9A82]">
          ...
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white rounded-2xl p-6 z-50">
            ...
            <div className="space-y-3 mb-6">
              {(() => {
                ...
                return statuses
                  .filter((s) => {
                    // Nếu đã chọn 'Đã xác nhận', 'Đang giao', hoặc 'Đã giao' thì ẩn 'Đã huỷ'
                    if (
                      (newStatus === "confirmed" ||
                        newStatus === "shipping" ||
                        newStatus === "delivered") &&
                  ...
                  .map((status) => {
                    // Không cho chọn lại các trạng thái trước trạng thái hiện tại
                    const statusIdx = statusOrder.indexOf(status.value);
                    const isDisabled =
                      currentIdx !== -1 && statusIdx < currentIdx;
                    return (
                      <label
                        key={status.value}
