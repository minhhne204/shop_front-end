...
const AdminBrands = () => {
  ...
  const validateForm = () => {
    ...
    if (!form.name.trim()) {
      ...
      newErrors.name = "Tên thương hiệu phải ít nhất 2 ký tự";
    } else if (form.name.length > 50) {
      newErrors.name = "Tên thương hiệu tối đa 50 ký tự";
    }
    ...
    // Logo
    if (!form.logo) {
      newErrors.logo = "Vui lòng tải logo thương hiệu";
    }
        ...
        <div className="flex items-center justify-center gap-2">
          ...
          {getPageNumbers().map((pageNum, idx) => (
            <button
              key={idx}
              onClick={() =>
                typeof pageNum === "number" && fetchBrands(pageNum)
              }
              disabled={pageNum === "..."}
