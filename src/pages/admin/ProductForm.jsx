...
const ProductForm = () => {
  ...
  const validateForm = () => {
    ...
    if (!form.hasVariants && Number(form.stock) < 0) {
      newErrors.stock = "Tồn kho không được âm";
    }

    if (form.hasVariants) {
      if (!form.variantType.trim()) {
        newErrors.variantType = "Vui lòng nhập loại biến thể";
      }
        ...
        <div className="bg-white rounded-2xl border border-[#EBEBEB] p-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              />
              {errors.name && (
                <p className="mt-1 text-[12px] text-[#C45C4A]">{errors.name}</p>
              )}
            </div>
            <div>
              <label className="block text-[13px] font-medium text-[#2D2D2D] mb-2">
            ...
            <>
              <div className="mb-4">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, variantType: e.target.value })
                  }
              ...
              {variants.length > 0 && (
                <div className="space-y-3 mb-4">
                  {variants.map((variant, index) => (
                    <div
                      key={index}
                      className="flex items-center justify-between p-4 bg-[#F5F5F3] rounded-xl"
                    >
                      <div className="flex-1">
                        <div className="flex items-center gap-3">
                          ...
                          {!variant.isActive && (
                            <span className="px-2 py-0.5 bg-[#FEF2F2] text-[#C45C4A] text-[11px] rounded-full">
                              Ẩn
                            </span>
                          )}
                        </div>
                        <div className="flex items-center gap-4 mt-1 text-[13px] text-[#6B6B6B]">
                          ...
                          {variant.salePrice && (
                            <span className="text-[#7C9A82]">
                              Sale: {formatPrice(variant.salePrice)}
                            </span>
                          )}
