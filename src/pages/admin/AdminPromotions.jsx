  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[24px] font-semibold text-[#2D2D2D]">Khuyến mãi</h1>
          <p className="text-[14px] text-[#6B6B6B] mt-1">{promotions.length} mã giảm giá</p>
        </div>
        <button
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#EBEBEB]">
                  <th className="text-left px-5 py-4 text-[13px] font-medium text-[#6B6B6B]">Mã</th>
                  <th className="text-left px-5 py-4 text-[13px] font-medium text-[#6B6B6B]">Giảm giá</th>
                  <th className="text-left px-5 py-4 text-[13px] font-medium text-[#6B6B6B]">Thời hạn</th>
                  <th className="text-left px-5 py-4 text-[13px] font-medium text-[#6B6B6B]">Đã dùng</th>
              ...
              <tbody>
                {promotions.map((promo) => (
                  <tr key={promo._id} className="border-b border-[#EBEBEB] last:border-0 hover:bg-[#FAFAF8]">
                    <td className="px-5 py-4">
                      <p className="text-[14px] font-mono font-medium text-[#7C9A82]">{promo.code}</p>
                      {promo.description && (
                        <p className="text-[12px] text-[#6B6B6B] mt-0.5">{promo.description}</p>
                      )}
                    </td>
                    <td className="px-5 py-4">
                      ...
                      {promo.minOrder > 0 && (
                        <p className="text-[12px] text-[#6B6B6B] mt-0.5">Từ {formatCurrency(promo.minOrder)}</p>
                      )}
                    </td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4">
                      {promo.isActive ? (
                        <span className="px-2.5 py-1 bg-green-100 text-green-700 text-[12px] font-medium rounded-lg">Hoạt động</span>
                      ) : (
                        <span className="px-2.5 py-1 bg-gray-100 text-gray-600 text-[12px] font-medium rounded-lg">Tắt</span>
                      )}
                    </td>
                    <td className="px-5 py-4">
        ...
        <>
          ...
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg bg-white rounded-2xl p-6 z-50 max-h-[90vh] overflow-y-auto">
            <h3 className="text-[18px] font-semibold text-[#2D2D2D] mb-5">
              {modal.mode === 'add' ? 'Thêm mã khuyến mãi' : 'Sửa mã khuyến mãi'}
            </h3>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[13px] font-medium text-[#2D2D2D] mb-2">Mã giảm giá</label>
                  <input
                    type="text"
                    value={form.code}
                ...
                <div>
                  <label className="block text-[13px] font-medium text-[#2D2D2D] mb-2">Loại giảm</label>
                  <select
                    value={form.type}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-[13px] font-medium text-[#2D2D2D] mb-2">Đơn tối thiểu</label>
                  <input
                    type="number"
                    value={form.minOrder}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[13px] font-medium text-[#2D2D2D] mb-2">Giảm tối đa</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-[13px] font-medium text-[#2D2D2D] mb-2">Số lượt dùng</label>
                  <input
                    type="number"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[13px] font-medium text-[#2D2D2D] mb-2">Ngày bắt đầu</label>
                  <input
                    type="date"
                ...
                <div>
                  <label className="block text-[13px] font-medium text-[#2D2D2D] mb-2">Ngày kết thúc</label>
                  <input
                    type="date"
                    value={form.endDate}
              ...
              <div className="flex items-center gap-3">
                ...
                />
                <label htmlFor="isActive" className="text-[14px] text-[#2D2D2D]">Kích hoạt mã</label>
              </div>
      ...
      {deleteModal && (
        <>
          ...
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white rounded-2xl p-6 z-50">
            <h3 className="text-[18px] font-semibold text-[#2D2D2D] mb-2">Xoá mã khuyến mãi</h3>
