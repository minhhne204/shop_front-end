    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[14px] text-[#6B6B6B] mt-1">{pagination.total} danh mục</p>
        </div>
        <button
            ...
            {categories.map((category) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[14px] font-medium text-[#2D2D2D]">{category.name}</p>
                  {category.description && (
                    <p className="text-[12px] text-[#6B6B6B] mt-0.5 line-clamp-1">{category.description}</p>
                  )}
                </div>
      ...
      {pagination.totalPages > 1 && (
        <div className="flex items-center justify-center gap-2">
          ...
          {getPageNumbers().map((pageNum, idx) => (
            <button
              key={idx}
              onClick={() => typeof pageNum === 'number' && fetchCategories(pageNum)}
              disabled={pageNum === '...'}
        ...
        <>
          ...
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white rounded-2xl p-6 z-50">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-[13px] font-medium text-[#2D2D2D] mb-2">Tên danh mục</label>
                <input
                  type="text"
                  value={form.name}
      ...
      {deleteModal && (
        <>
          ...
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white rounded-2xl p-6 z-50">
            <h3 className="text-[18px] font-semibold text-[#2D2D2D] mb-2">Xoá danh mục</h3>
