...
const AdminOrderDetail = () => {
  ...
  const statuses = [
    { value: 'pending', label: 'Chờ xử lý', color: 'bg-yellow-100 text-yellow-700', icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z' },
    { value: 'confirmed', label: 'Đã xác nhận', color: 'bg-blue-100 text-blue-700', icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' },
    { value: 'shipping', label: 'Đang giao', color: 'bg-purple-100 text-purple-700', icon: 'M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10a1 1 0 001 1h1m8-1a1 1 0 01-1 1H9m4-1V8a1 1 0 011-1h2.586a1 1 0 01.707.293l3.414 3.414a1 1 0 01.293.707V16a1 1 0 01-1 1h-1m-6-1a1 1 0 001 1h1M5 17a2 2 0 104 0m-4 0a2 2 0 114 0m6 0a2 2 0 104 0m-4 0a2 2 0 114 0' },
    { value: 'delivered', label: 'Đã giao', color: 'bg-green-100 text-green-700', icon: 'M5 13l4 4L19 7' },
    { value: 'cancelled', label: 'Đã huỷ', color: 'bg-red-100 text-red-700', icon: 'M6 18L18 6M6 6l12 12' }
  ]
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        <span className={`px-4 py-2 rounded-xl text-[14px] font-medium ${statusInfo.color}`}>
          {statusInfo.label}
        </span>
      </div>
      ...
      {order.status !== 'cancelled' && (
        <div className="bg-white rounded-2xl border border-[#EBEBEB] p-6">
          <h2 className="text-[16px] font-semibold text-[#2D2D2D] mb-6">Tiến trình đơn hàng</h2>
          <div className="flex items-center justify-between">
              ...
              return (
                <div key={status.value} className="flex flex-col items-center flex-1">
                  <div className="relative w-full flex items-center justify-center">
                    {index > 0 && (
                      <div className={`absolute left-0 right-1/2 h-1 top-1/2 -translate-y-1/2 ${index <= currentStatusIndex ? 'bg-[#7C9A82]' : 'bg-[#EBEBEB]'}`} />
                    )}
                    {index < statuses.filter(s => s.value !== 'cancelled').length - 1 && (
                      <div className={`absolute left-1/2 right-0 h-1 top-1/2 -translate-y-1/2 ${index < currentStatusIndex ? 'bg-[#7C9A82]' : 'bg-[#EBEBEB]'}`} />
                    )}
                    <div className={`relative z-10 w-12 h-12 rounded-full flex items-center justify-center ${isActive ? 'bg-[#7C9A82] text-white' : 'bg-[#F5F5F3] text-[#9A9A9A]'} ${isCurrent ? 'ring-4 ring-[#7C9A82]/20' : ''}`}>
                      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={status.icon} />
                      </svg>
                    </div>
                  </div>
                  <p className={`text-[12px] mt-3 font-medium ${isActive ? 'text-[#2D2D2D]' : 'text-[#9A9A9A]'}`}>{status.label}</p>
                </div>
              )
            })}
          </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-2xl border border-[#EBEBEB] p-6">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-[16px] font-semibold text-[#2D2D2D]">Sản phẩm ({order.items?.length || 0})</h2>
            </div>
            <div className="space-y-4">
              {order.items?.map((item, index) => (
                <div key={index} className="flex gap-4 p-4 bg-[#FAFAF8] rounded-xl">
                  <img
              ...
              <div className="space-y-3">
                ...
                {order.discount > 0 && (
                  <div className="flex justify-between text-[14px]">
                    ...
                    <span className="text-[#7C9A82]">-{formatCurrency(order.discount)}</span>
                  </div>
                )}
                <div className="flex justify-between pt-3 border-t border-[#EBEBEB]">
                  ...
                  <span className="text-[18px] font-semibold text-[#C45C4A]">{formatCurrency(order.totalAmount)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          {order.status !== 'delivered' && order.status !== 'cancelled' && (
            <div className="bg-white rounded-2xl border border-[#EBEBEB] p-6">
              <h2 className="text-[16px] font-semibold text-[#2D2D2D] mb-5">Cập nhật trạng thái</h2>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-2xl border border-[#EBEBEB] p-6">
            <div className="flex items-center gap-3 mb-5">
              <div className="w-10 h-10 rounded-full bg-[#7C9A82] flex items-center justify-center text-white text-[14px] font-medium">
                {order.shippingAddress?.fullName?.charAt(0)?.toUpperCase() || 'K'}
              </div>
              <div>
                <h2 className="text-[15px] font-semibold text-[#2D2D2D]">{order.shippingAddress?.fullName}</h2>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[14px] text-[#2D2D2D]">{order.shippingAddress?.phone || 'Chưa có'}</p>
                </div>
              </div>
              {(order.shippingAddress?.email || order.user?.email) && (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[14px] text-[#2D2D2D]">{order.shippingAddress?.email || order.user?.email}</p>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="bg-white rounded-2xl border border-[#EBEBEB] p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-[15px] font-semibold text-[#2D2D2D]">Địa chỉ giao hàng</h2>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-[#EBEBEB] p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-[15px] font-semibold text-[#2D2D2D]">Thanh toán</h2>
            </div>
          ...
          {order.note && (
            <div className="bg-white rounded-2xl border border-[#EBEBEB] p-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-[15px] font-semibold text-[#2D2D2D]">Ghi chú</h2>
              </div>
              <p className="text-[14px] text-[#6B6B6B] bg-[#F5F5F3] p-4 rounded-xl">{order.note}</p>
            </div>
          )}
        </div>
      </div>
